...
def translational_symmetry_minus(pathdir, filename, err_sym_minus_factor=-1):
    try:
        ...
        with torch.no_grad():
            ...
            if err_sym_minus_factor==-1:
                error_threshold = 7*rmse_loss(model(factors),product)
            else:
                error_threshold = err_sym_minus_factor*rmse_loss(model(factors),product)
