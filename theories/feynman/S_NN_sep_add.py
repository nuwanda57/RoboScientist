...
def NN_sep_add(pathdir, filename, err_sep_add_factor=-1):
    try:
        ...
        with torch.no_grad():
            ...
            if err_sep_add_factor==-1:
                error_threshold = 10*rmse_loss(model(factors),product)
            else:
                error_threshold = err_sep_add_factor*rmse_loss(model(factors),product)
            ...
            for i in range(1,n_variables):
                ...
                for j in c:
                    ...
                    per_error = torch.sqrt(torch.mean((model(facts)-sm+model(fact_vary))**2))/torch.sqrt(torch.mean(model(facts)**2))
                    print(per_error)
