...
def translational_symmetry_plus(pathdir, filename, err_sym_plus_factor=-1):
    try:
        ...
        with torch.no_grad():
            ...
            #a = 1
            a = 0.5
            if err_sym_plus_factor==-1:
                error_threshold = 7*rmse_loss(model(factors),product)
            else:
                error_threshold = err_sym_plus_factor*rmse_loss(model(factors),product)
