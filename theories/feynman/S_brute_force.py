...
from theories.feynman.S_readable_formulas_from_bruteForce_v1 import readable_formulas_from_bruteForce_v1
from theories.feynman.S_readable_formulas_from_bruteForce_v2 import readable_formulas_from_bruteForce_v2


# sep_type = 3 for add and 2 for mult and 1 for normal
def brute_force(pathdir, filename, methods_tried, method_name, BF_try_time, BF_ops_file_type, sep_type, use_MDL,
                check_prefactor):
    ...
    if os.stat("bruteformulas.txt").st_size != 0:
        ...
        if (sep_type == 2 or sep_type == 3) and check_prefactor == 1:
            ...
            BF_prefactor_unsolved = np.loadtxt(
                "bruteprefactor.txt")  # use this if the BF doesn't find an expression for the prefactor
