...
def translational_symmetry_multiply(pathdir, filename, err_sym_mult_factor=-1):
    try:
        ...
        with torch.no_grad():
            ...
            a = 1.2
            if err_sym_mult_factor==-1:
                error_threshold = 7*rmse_loss(model(factors),product)
            else:
                error_threshold = err_sym_mult_factor*rmse_loss(model(factors),product)
            ...
            for i in range(0,n_variables,1):
                for j in range(0,n_variables,1):
                    if i<j:
                        ...
                        error = torch.sqrt(torch.mean((product-model(fact_translate))**2))/torch.sqrt(torch.mean(product**2))
                        print("ERROR: ", abs(error))
                        if abs(error) < error_threshold:
                            file_name = filename + "-translated_mult"
                            ...
                            try:
                                os.mkdir("results/translated_data_mult/")
                            except:
                                pass
                            np.savetxt("results/translated_data_mult/"+file_name , data_translated)
                            print("SUCCESS", i, j)
