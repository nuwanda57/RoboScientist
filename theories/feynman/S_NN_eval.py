...
def NN_eval(pathdir, filename):
    try:
        ...
        product = torch.from_numpy(f_dependent[0:100000])
        if is_cuda:
        ...
        factors_val = torch.from_numpy(variables[100000:120000])
        if is_cuda:
            factors_val = factors_val.cuda()
        else:
            factors_val = factors_val
        factors_val = factors_val.float()
        product_val = torch.from_numpy(f_dependent[100000:120000])
        if is_cuda:
